...
/* ------------------------------------ */
if ( ! function_exists( 'cutslice_hex2rgb' ) ) {

	function cutslice_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'cutslice_enqueue_google_fonts' ) ) {

	function cutslice_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'cutslice_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'cutslice_dynamic_css' ) ) {

	function cutslice_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = cutslice_hex2rgb($color_1);
...
.author-bio .bio-avatar:after { background: '.esc_attr( get_theme_mod('color-1') ).'; }
				'."\n";
			}
			// gradient left
			if ( get_theme_mod('gradient-left','#ff9966') != '#ff9966' ) {
				$styles .= '
...
.post-comments > span:before { border-color: transparent '.esc_attr( get_theme_mod('gradient-left') ).' transparent transparent; }
			...
			// gradient right
			if ( get_theme_mod('gradient-right','#ff5e62') != '#ff5e62' ) {
				$styles .= '
.leftbar-outer .social-links,
.wrap-nav,
.sidebar .post-nav li a:hover,
...
.post-comments { background: linear-gradient(90deg, '.esc_attr( get_theme_mod('gradient-left') ).' 0%, '.esc_attr( get_theme_mod('gradient-right') ).' 100%); }
			...
			// sidebar color
			if ( get_theme_mod('color-sidebar','#000000') != '#000000' ) {
				$styles .= '
#header,
#header:after { background: '.esc_attr( get_theme_mod('color-sidebar') ).'; }
@media only screen and (max-width: 960px) {
	.s1 { background: '.esc_attr( get_theme_mod('color-sidebar') ).'; }
			...
			wp_add_inline_style( 'cutslice-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'cutslice_dynamic_css' );
