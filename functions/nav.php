...
class Nav {
	 ...
	 * @var string
	 */
	protected $handle_prefix = 'cutslice-nav';
	...
	public function init() {
		if ( apply_filters( 'cutslice_disable_nav_mods', false ) ) {
			return;
		}
	...
	public static function nav_menu( $args, $toggle_label = false, $nav_classes = 'main-navigation nav-menu' ) {
		if ( false === $toggle_label ) {
			$toggle_label = '<span class="screen-reader-text">' . esc_html__( 'Expand Menu', 'cutslice' ) . '</span><div class="menu-toggle-icon"><span></span><span></span><span></span></div>';
		}
		?>
	...
	public function nav_menu_item_title( $title, $item, $args, $depth ) {
		...
		if ( $is_current ) {
			...
			return sprintf(
				...
				__( '<span class="screen-reader-text">%1$s </span>%2$s', 'cutslice' ),
				esc_html__( 'Current Page:', 'cutslice' ),
				$title
			);
		}
		...
		if ( $is_current_parent ) {
			...
			return sprintf(
				...
				__( '<span class="screen-reader-text">%1$s </span>%2$s', 'cutslice' ),
				esc_html__( 'Current Page Parent', 'cutslice' ),
				$title
			);
		}
